...
| `--run-sdust` | Drop candidates made mostly of low-complexity sequence, early — see [§3.1](#31---run-sdust-dropping-low-complexity-false-positives). Off by default. |


### 3.1 `--run-sdust`: dropping low-complexity false positives

Satellite and microsatellite arrays can present a convincing pair of "LTRs"
flanking a "body", and a few of them survive into the final table as false
positives. `--run-sdust` catches them by running
[sdust](https://github.com/lh3/sdust) over each full-length candidate and
dropping the ones that come back mostly masked:

```bash
bash ../module2/ltrharvest_wrapper2.sh \
  --genome ../test/Athal_tair10_chr2.fa.gz --threads 20 --run-sdust
```

The gate sits right after the LTRharvest/LTRfinder merge and **before**
TEsorter2, so the elements it removes never cost a classification, a TSD scan,
or a clustering pass. sdust is installed into the run's `_tools/` directory on
first use, alongside TRF-mod and the rest.

| Flag | Meaning |
|---|---|
| `--run-sdust` | Turn the filter on. Default: off, and a run without it is byte-identical to one from before the feature existed. |
| `--sdust-args` | Args for sdust itself. Only `-w INT` (window) and `-t INT` (score threshold) exist. Default `'-w 64 -t 15'` — sdust's own default is `-t 20`, so this masks a little more aggressively. |
| `--max-dust-frac` | Drop a candidate when this fraction or more of it is masked. Default `0.50` — see [§3.2](#32-choosing---max-dust-frac). |

**Nested elements do not count**, in either direction. The fraction is taken
over ACGT bases only, and from round 2 on a nested inner arrives already
IUPAC-masked in the round's genome (see [§4.2](#42-depthncleanltrfa-format--nested-regions-are-masked)); sdust
treats every non-ACGT base as a hard break and never reports one as masked. So
`--max-dust-frac 0.50` means *50% of the element's own sequence*, and a real
element is not condemned by what is nested inside it — nor is a satellite array
rescued by one.

Each round writes `<prefix>_r<N>.work/<prefix>_r<N>.sdust.tsv`, one row per
candidate, so every drop is checkable:

```
#name             ret_len  acgt_bp  dust_bp  dust_frac  verdict
Chr2:1234-5678    4445     4445     41       0.0092     keep
Chr2:98000-99500  1501     1501     1501     1.0000     drop
```


### 3.2 Choosing `--max-dust-frac`

The default `0.55` is calibrated against curated ground truth rather than picked
by feel, using two independent sets.

**Curated intact LTR-RTs** — Shujun Ou's TAIR12-TE (146), riceTElib (387) and
MTEC maize (572), 1105 elements:

| percentile | dust fraction |
|---|---|
| median | 0.032 |
| p99 | 0.213 |
| **max** | **0.333** |

The maize 572 matter most: they are real genomic copies with genuinely diverged
LTRs, whereas the Arabidopsis and rice entries were reconstituted from split
`_INT`/`_LTR` downloads and carry identical LTR pairs. The ceiling is therefore
set by real sequence.

**A broad cross-species database** (20703 elements) gives a second, larger check.
Restricted to its 3951 plant entries, the dustiest is **0.5205**.

`0.55` clears both ceilings — zero loss on either set — with ~0.03 of margin.

| threshold | curated (1105) lost | plants (3951) lost |
|---|---|---|
| 0.80 | 0 | 0 |
| 0.60 | 0 | 0 |
| **0.55** | **0** | **0** |
...
**Raise it for AT-rich genomes.** Everything above 0.80 in that database is a
RIP-degraded Ascomycete fungal element (*Paecilomyces*, *Trichoderma*,
*Penicillium* …) at 16-23% GC — genuine full-length Ty3/Copia that sdust reads as
low-complexity because repeat-induced point mutation has hypermutated them toward
AT. Across the whole database `r(dust_frac, GC) = -0.374`. Use `0.80` or higher
for such genomes. Basidiomycete rusts are unaffected: 658 *Puccinia* entries top
out at 0.262.

Note the metric is composition-agnostic — it flags GC-rich low complexity too
(the dustiest plant entry is a 69.5%-GC wheat Copia), so "AT-rich" is the common
case, not the only one.

The per-round `<prefix>_r<N>.work/<prefix>_r<N>.sdust.tsv` report gives the full
per-candidate distribution, which is the reliable way to set this for a specific
assembly.
